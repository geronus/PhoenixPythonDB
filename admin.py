    ...
    def get(self):
        ...
        for person in members:
            ...
            if person.rank_new == True:
              rank_dict = {'id': person.key.id(),
                           'name': person.username,
                           'gdp': person.gdp,
                           'rank': person.rank}

              rank_tracker.append(person)
...
class Admin_Submit(webapp2.RequestHandler):
    ...
    def post(self):
        ...
        if action == "newcontest":
            ...
            updates = self.request.GET.getall('member')

            for identifier in updates:
                try:
                    candidate_key = ndb.Key(identifier)
                except:
                    candidate_key = None
                    debug.error("Unable to retrieve Member key from POST request!")
